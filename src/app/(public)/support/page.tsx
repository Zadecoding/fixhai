...
import {
  ...
  MessageSquare, Clock, Shield, Zap, Phone, Mail,
} from "lucide-react";
...
const FAQ = [
  ...
  { q: "My technician didn't show up.",         a: "Raise a Critical ticket here with your Booking ID. We'll process a full ₹99 refund within 5–7 business days." },
  { q: "When will I get my refund?",            a: "Approved refunds are credited to your original payment method within 5–7 business days." },
  { q: "Can I reschedule a booking?",           a: "Currently you can cancel and re-book. Rescheduling is coming soon — raise a ticket and we'll assist manually." },
  { q: "How do I download my invoice?",         a: "Invoices for completed bookings are in Dashboard → Bookings → Download Invoice." },
];

const contactMethods = [
  { icon: Phone,        title: "Call Us",    detail: "+91 98765 43210",   subtext: "Mon–Sat, 9AM–6PM",    href: "tel:+919876543210",             color: "bg-blue-100    dark:bg-blue-900/30    text-blue-600"    },
  { icon: Mail,         title: "Email Us",   detail: "support@fixhai.in", subtext: "Reply within 24 hrs", href: "mailto:support@fixhai.in",       color: "bg-green-100   dark:bg-green-900/30   text-green-600"   },
  { icon: MessageSquare,title: "WhatsApp",   detail: "+91 98765 43210",   subtext: "Quick responses",     href: "https://wa.me/919876543210",     color: "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600" },
];
...
export default function SupportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.subject.trim() || !form.description.trim()) { toast.error("Please fill in subject and description."); return; }
    setLoading(true);
    const res = await submitSupportTicket({ subject: form.subject, description: form.description, priority: form.priority, booking_id: form.booking_id || undefined });
    setLoading(false);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
          ...
          <div className="grid sm:grid-cols-3 gap-4 mt-6">
            {contactMethods.map((m) => {
              const Icon = m.icon;
              return (
                <a key={m.title} href={m.href} target={m.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer">
                  <Card hoverable className="p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-[var(--muted-foreground)]">{m.subtext}</div>
                    </div>
                  </Card>
                </a>
              );
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <AnimatePresence mode="wait">
            {submitted ? (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center text-center py-16">
                ...
                <p className="text-[var(--muted-foreground)] max-w-sm mb-6">We've received your request and will respond within 2 hours. Check your email for confirmation.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {PRIORITIES.map((p) => (
                        <button type="button" key={p.value} onClick={() => setForm(f => ({ ...f, priority: p.value }))}
                          className={`text-left px-4 py-3 rounded-xl border-2 transition-all ${form.priority === p.value ? p.color + " border-current" : "border-[var(--border)] bg-[var(--card)] hover:border-orange-300"}`}>
                  ...
                  <div>
                    ...
                    <input id="subject" type="text" value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-[var(--border)] bg-[var(--card)] text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 transition-shadow" required />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea id="description" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                      placeholder="Please describe your issue in as much detail as possible..." rows={5}
                      className="w-full px-4 py-3 rounded-xl border border-[var(--border)] bg-[var(--card)] text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 transition-shadow resize-none" required />
                  </div>
                  ...
                  <div>
                    ...
                    <input id="booking_id" type="text" value={form.booking_id} onChange={e => setForm(f => ({ ...f, booking_id: e.target.value }))}
        ...
        <div className="lg:col-span-2">
            ...
            {FAQ.map((item, i) => (
              <Card key={i} className="overflow-hidden">
                <button className="w-full text-left px-5 py-4 flex items-center justify-between gap-3" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  ...
                  <ChevronRight className={`w-4 h-4 text-[var(--muted-foreground)] shrink-0 transition-transform duration-200 ${openFaq === i ? "rotate-90" : ""}`} />
                </button>
                <AnimatePresence>
                  {openFaq === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <p className="px-5 pb-4 text-sm text-[var(--muted-foreground)] leading-relaxed border-t border-[var(--border)] pt-3">{item.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </Card>
          ...
          <Card className="p-5 mt-5 bg-orange-50 dark:bg-orange-950/20 border-orange-200 dark:border-orange-800">
            ...
            <a href="tel:+919876543210" className="text-sm font-semibold text-orange-600 hover:underline">📞 +91 98765 43210</a>
          </Card>
        </div>
      </div>
    </div>
